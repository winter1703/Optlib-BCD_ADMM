...
--admm解决问题的基本形式
class Opt_problem [FiniteDimensional ℝ E₁] [FiniteDimensional ℝ E₂] [FiniteDimensional ℝ F] where
   f₁ : E₁ → ℝ
   f₂ : E₂ → ℝ
   A₁ : E₁ →L[ℝ] F
   A₂ : E₂ →L[ℝ] F
...
--增广lagrange函数
def Augmented_Lagrangian_Function (opt : Opt_problem E₁ E₂ F)(ρ : ℝ): E₁ × E₂ × F → ℝ :=
   fun (x₁ , x₂ , y) =>
   (opt.f₁ x₁) + (opt.f₂ x₂) +
   inner y  ((opt.A₁ x₁) + (opt.A₂ x₂) - opt.b) + ρ / 2 * ‖(opt.A₁ x₁) + (opt.A₂ x₂) - opt.b‖^2

--ADMM的基本迭代格式
class ADMM extends (Opt_problem E₁ E₂ F) where
   x₁ : ℕ → E₁
   x₂ : ℕ → E₂
   y  : ℕ → F
   ρ  : ℝ
...
--凸的kkt条件
variable {E₁ E₂ F} in
class Convex_KKT (x₁ : E₁ )(x₂ : E₂)(y : F) (opt : Opt_problem E₁ E₂ F) :Prop where
