...
variable {X Y:Type*}
[NormedAddCommGroup X] [InnerProductSpace ℝ X] [CompleteSpace X] [ProperSpace X] [FiniteDimensional ℝ X]
[NormedAddCommGroup Y] [InnerProductSpace ℝ Y] [CompleteSpace Y] [ProperSpace Y] [FiniteDimensional ℝ Y]
(A : X →L[ℝ] Y)(fullrank: Injective A)

lemma KerA_bot : ker A = ⊥ := ker_eq_bot.2 fullrank
...
lemma KerA'A_bot : ker (A†.comp A) = ⊥ := by
   rw[← KerA_eq_KerA'A]
   apply KerA_bot A fullrank

--A单射推出A'A为单射
lemma A'A_inj : Injective (A†.comp  A) := ker_eq_bot.1 (KerA'A_bot A fullrank)

--A'A为单射推出A'A为满射（X有限维）
lemma A'A_Sur : Surjective (A†.comp A) := by
   have H:FiniteDimensional.finrank ℝ X = FiniteDimensional.finrank ℝ X:=rfl
   apply (injective_iff_surjective_of_finrank_eq_finrank H).1 (A'A_inj A fullrank)

--A'A的逆为有界线性映射（X有限维）
lemma inv_bounded₁ :∃ C > 0 , ∀ x : X , ‖x‖ ≤ C * ‖(A†.comp A) x‖ := by
