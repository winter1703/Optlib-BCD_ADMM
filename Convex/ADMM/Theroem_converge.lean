...
--dyx xzx
--dyx xzx
lemma nonneg_prime : 1 + τ - τ ^ 2 > 0 := by
   ...
   have h6 : |τ - 2⁻¹| < √5/2 := by
      ...
      have h7 : |θ| < √5/2 := by
         ...
         have h9 : -(√5/2) < θ := by
            have g2 : 1/2 < √5/2 := by
               rw[mul_self_lt_mul_self_iff];norm_num;rw[← pow_two]
               repeat norm_num
               have g3: 0 ≤ √5 := by apply Real.sqrt_nonneg
               apply div_nonneg g3;norm_num
            have h10 : θ > -(√5/2) := by
               calc
                  _ = τ - 2⁻¹ := by rfl
...
lemma nonneg₃ : max (1 - τ) (1 - 1 / τ) ≥ 0 := by
   ...
   have hb : 1 - 1/τ ≥ 1 - τ := by
      have : 1 / τ ≤ 1 := by
         rw [one_div]
         apply inv_le_one; linarith
      linarith
   rw [max_eq_right hb];apply sub_nonneg_of_le
   rw [one_div];apply inv_le_one; linarith

lemma Φ_is_nonneg : ∀ n : ℕ , Φ n ≥ 0 := by
   intro n
   rcases admm.htau with ⟨h1, _⟩
   simp [Φ]
   have h2 : Ψ n ≥ 0 := by
...
-- byf
lemma Φ_bd_above : ∃ C : ℝ, ∀ n : ℕ, Φ n < C := by
...
--lhj
--ey是有界序列
lemma ey_isBounded' : ∃ (r : ℝ), (range ey) ⊆ ball 0 r := by
...
lemma max_tau_mul_nonneg :∀ n, 0 ≤ max (1 - τ) (1 - 1 / τ) * ρ * ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2 :=by
   intro n
   apply mul_nonneg
   · apply mul_nonneg
...
lemma Φn_isBounded : ∃ r_Φ , ∀ n, ‖Φ n‖ < r_Φ := by
...
--byf
lemma open_mapping_e₁: ∃ C > 0, ∀ n : ℕ, ‖e₁ n‖ ≤ C * ‖A₁ (e₁ n)‖ := by
...
--byf
--x₁这个序列是有界的
lemma x₁_isBounded' : ∃ (r : ℝ), (range x₁) ⊆ ball 0 r := by
...
-- byf
--y这个序列是有界的
lemma y_isBounded' :  ∃ (r : ℝ), (range y) ⊆ ball 0 r := by
...
--子列映射是严格单增
lemma hphi_StrictMono : StrictMono φ := (Subseq fullrank₁ fullrank₂).hphi
...
lemma hconverge : Tendsto (fun n => (x₁ (φ n),  x₂ (φ n), y (φ n)))
atTop (𝓝 x₁'' ×ˢ 𝓝 x₂'' ×ˢ 𝓝 y''):=by
   have := (Subseq fullrank₁ fullrank₂).hconverge
   rw[admm_nhds_prod_eq] at this
   exact this

lemma x₁_subseq_converge : Tendsto (fun n => (x₁ (φ n)))  atTop (𝓝 x₁'') :=
...
lemma y_subseq_converge  : Tendsto (fun n => (y (φ n)))  atTop (𝓝 y'') :=
...
lemma x₁_subseq_converge' : Tendsto (fun n => (x₁ (φ₁ n)))  atTop (𝓝 x₁'') :=by
...
lemma Φ_Summable₁' :∃ (c : ℝ) , ∀ (n : ℕ), ∑ i ∈ Finset.range n, ((Φ i.succ ) - (Φ (i.succ + 1))) ≤ c
:= by
...
lemma Φ_isSummable : Summable (fun n : ℕ => (Φ (n+1) ) - (Φ (n+1 + 1))) := by
...
lemma Φ_inequ₁(m : ℕ+):(min 1 (1 + 1 / τ - τ )) * ρ * ‖A₁ (e₁ (m+1)) + A₂ (e₂ (m+1))‖ ^ 2 ≤ Φ m - Φ (m + 1) := by
...
lemma Summable₁' :  ∀ (n : ℕ), (fun n => (min 1 (1 + 1 / τ - τ )) * ρ * ‖A₁ (e₁ (n+2)) + A₂ (e₂ (n+2))‖^2)  n
...
lemma  Φ_inequ₂(m : ℕ+) : (min τ (1 + τ - τ ^ 2)) * ρ * ‖A₂ (x₂ (m + 1) - x₂ m)‖^2 ≤ Φ m - Φ (m + 1) := by
...
lemma Summable₂' :  ∀ (n : ℕ), (fun n =>(min τ (1 + τ - τ ^ 2)) * ρ * ‖A₂ (x₂ (n + 1+ 1) - x₂ (n+1))‖^2) n
≤ (fun n : ℕ => (Φ (n+1) ) - (Φ (n+1 + 1))) n :=by
   intro n
   simp only
   let m := (n+1).toPNat'
...
-- byf
lemma y_subseq_converge' : Tendsto (fun n => (y (φ₁ n))) atTop (𝓝 y'') := by
...
-- byf
lemma square_converge_zero₁ (h : Tendsto (fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2)  atTop (𝓝 0)) :
...
lemma square_converge_zero₃ (h : Tendsto (fun n => ‖A₂ (x₂ (n + 1) - x₂ n)‖ ^ 2)  atTop (𝓝 0)) :
...
lemma converge_zero₃ : Tendsto (fun n => ‖A₂ (x₂ (n + 1) - x₂ n)‖)  atTop (𝓝 0) := by
...
lemma Seq_converge_zero₁ : Tendsto (fun n => A₁ (e₁ n) + A₂ (e₂ n))  atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.2 converge_zero₁

-- A₁ (x₁ n) + A₂ (x₂ n) - b → 0
lemma Seq_converge_zero₂ : Tendsto (fun n => A₁ (x₁ n) + A₂ (x₂ n) - b)  atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.2 converge_zero₂

-- A₂ (x₂ (n + 1) - x₂ n) → 0
lemma Seq_converge_zero₃ : Tendsto (fun n => A₂ (x₂ (n + 1) - x₂ n))  atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.2 converge_zero₃

-- byf
-- A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n)) → 0
lemma sub_Seq_converge_zero₁ : Tendsto (fun n => A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n)))
atTop (𝓝 0) := by
...
-- byf
-- A₁ (x₁ (φ₁ n)) + A₂ (x₂ (φ₁ n)) - b → 0
lemma sub_Seq_converge_zero₂ : Tendsto (fun n => A₁ (x₁ (φ₁ n)) + A₂ (x₂ (φ₁ n)) - b) atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converge_zero₂
   apply StrictMono.tendsto_atTop
   have : (fun n => (Int.toNat (φ₁ n))) = (fun n => (φ (n+1))) := by
      ext n; rw [φ₁_equ fullrank₁ fullrank₂ n]; simp
...
-- byf
-- A₂ (x₂ ((φ₁ n) + 1) - x₂ (φ₁ n)) → 0
lemma sub_Seq_converge_zero₃ : Tendsto (fun n => A₂ (x₂ ((φ₁ n) + 1) - x₂ (φ₁ n))) atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converge_zero₃
   apply StrictMono.tendsto_atTop
   have : (fun n => (Int.toNat (φ₁ n))) = (fun n => (φ (n+1))) := by
      ext n; rw [φ₁_equ fullrank₁ fullrank₂ n]; simp
...
-- A₂ (x₂ (φ₁ n) - x₂ ((φ₁ n) - 1)) → 0
lemma sub_Seq_converge_zero₃' : Tendsto (fun n => A₂ (x₂ (φ₁ n) - x₂ ((φ₁ n) - 1))) atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converge_zero₃
   apply StrictMono.tendsto_atTop
...
lemma const_smul_subseq₁: Tendsto (fun n => (( 1 - τ) * ρ ) • (A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n)))) atTop (𝓝 0) := by
   rw [← smul_zero (( 1 - τ) * ρ )]
   apply Filter.Tendsto.const_smul (sub_Seq_converge_zero₁ fullrank₁ fullrank₂) (( 1 - τ) * ρ)

-- byf
-- ρ • A₂ (x₂ ((φ₁ n) - 1) - x₂ (φ₁ n)) → 0
lemma const_smul_subseq₂ : Tendsto (fun n => ρ • A₂ (x₂ ((φ₁ n) - 1) - x₂ (φ₁ n))) atTop (𝓝 0) := by
...
lemma u_subseq_converge : Tendsto (fun n => u (φ₁ n)) atTop (𝓝 (- A₁† y'')) := by
...
lemma v_subseq_converge : Tendsto (fun n => v (φ₁ n)) atTop (𝓝 (- A₂† y'')) := by
...
lemma A₁'y_inthesubgradient : - A₁† y'' ∈ SubderivAt f₁ x₁'':=
   ...
   (fun n => x₁ (φ₁ n)) (x₁_subseq_converge' fullrank₁ fullrank₂)
   (fun n => u  (φ₁ n)) (u_subseq_converge   fullrank₁ fullrank₂)
   admm.lscf₁ (fun n => u_inthesubgradient (φ₁ n))

lemma A₂'y_inthesubgradient : - A₂† y'' ∈ SubderivAt f₂ x₂'':=
   Image_subgradient_closed f₂ x₂'' (- A₂† y'')
   (fun n => x₂ (φ₁ n)) (x₂_subseq_converge' fullrank₁ fullrank₂)
   (fun n => v  (φ₁ n)) (v_subseq_converge   fullrank₁ fullrank₂)
   admm.lscf₂ (fun n => v_inthesubgradient (φ₁ n))

--dyx
-- lim ‖ x_n ‖ = ‖ lim x_n ‖
lemma Satisfying_equational_constraint1': Tendsto A₁ (𝓝 x₁'') (𝓝 (A₁ x₁'')) := by
   apply Continuous.tendsto
   apply ContinuousLinearMap.continuous

lemma Satisfying_equational_constraint1:
...
lemma Satisfying_equational_constraint2': Tendsto A₂ (𝓝 x₂'') (𝓝 (A₂ x₂'')) := by
   apply Continuous.tendsto
   apply ContinuousLinearMap.continuous

lemma Satisfying_equational_constraint2:
Tendsto (fun n => A₂ (x₂ (φ n))) atTop (𝓝 (A₂ x₂'')) := by
...
lemma Satisfying_equational_constraint':
...
lemma subconverge_zero₂ : Tendsto (fun n =>  ‖A₁ (x₁ (φ n)) + A₂ (x₂ (φ n)) - b‖)  atTop (𝓝 0)
:= by
   apply tendsto_iff_seq_tendsto.1 converge_zero₂
   apply StrictMono.tendsto_atTop
   apply hphi_StrictMono

lemma Satisfying_equational_constraint_norm :  ‖(A₁ x₁'') + (A₂ x₂'') - admm.b‖ = 0 := by
   apply tendsto_nhds_unique (Satisfying_equational_constraint' fullrank₁ fullrank₂)
   apply subconverge_zero₂

lemma Satisfying_equational_constraint : (A₁ x₁'') + (A₂ x₂'') = admm.b := by
...
variable [Setting E₁ E₂ F admm (admm_kkt₁ fullrank₁ fullrank₂ (admm_kkt := admm_kkt) (_s := ⟨⟩))]

--byf
-- e₁ (φ n) → 0
-- x₁ (φ n) → (admm_kkt₁ fullrank₁ fullrank₂).x₁ = x₁''
lemma e₁_subseq_converge_zero : Tendsto (e₁ ∘ φ) atTop (𝓝 0) := by
...
lemma ey_subseq_converge_zero : Tendsto (ey ∘ φ) atTop (𝓝 0) := by
...
-- byf
-- ‖ey (φ n)‖ → 0
lemma nrm_ey_subseq_converge_zero : Tendsto (fun n => ‖ey (φ n)‖) atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.1; apply ey_subseq_converge_zero

-- ‖ey (φ n)‖^2 → 0
lemma sqnrm_ey_subseq_converge_zero : Tendsto (fun n => ‖ey (φ n)‖^2) atTop (𝓝 0) := by
   rw [← zero_pow]; apply Filter.Tendsto.pow ; apply nrm_ey_subseq_converge_zero; linarith

-- (1 / (τ * ρ)) * ‖ey (φ n)‖^2 → 0
lemma const_sqnrm_ey_subseq_converge_zero : Tendsto (fun n => (1 / (τ * ρ)) * ‖ey (φ n)‖^2) atTop (𝓝 0) := by
   rw [← mul_zero]
   apply Filter.Tendsto.const_mul (1 / (τ * ρ)) (sqnrm_ey_subseq_converge_zero fullrank₁ fullrank₂)

-- byf
-- A₁ (e₁ (φ n)) → 0
lemma A₁e₁_subseq_converge_zero : Tendsto (fun n => A₁ (e₁ (φ n))) atTop (𝓝 0) := by
...
-- ‖A₂ (e₂ (φ n))‖ → 0
lemma nrm_A₂e₂_subseq_converge_zero : Tendsto (fun n => ‖A₂ (e₂ (φ n))‖) atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.1; apply A₂e₂_subseq_converge_zero

-- ‖A₂ (e₂ (φ n))‖^2 → 0
lemma sqnrm_A₂e₂_subseq_converge_zero : Tendsto (fun n => ‖A₂ (e₂ (φ n))‖^2) atTop (𝓝 0) := by
   rw [← zero_pow]; apply Filter.Tendsto.pow ; apply nrm_A₂e₂_subseq_converge_zero; linarith

-- ρ * ‖A₂ (e₂ (φ n))‖^2 → 0
lemma const_sqnrm_A₂e₂_subseq_converge_zero : Tendsto (fun n => ρ * ‖A₂ (e₂ (φ n))‖^2) atTop (𝓝 0) := by
   rw [← mul_zero]
   apply Filter.Tendsto.const_mul ρ (sqnrm_A₂e₂_subseq_converge_zero fullrank₁ fullrank₂)

-- byf
-- A₁ (e₁ (φ n)) + A₂ (e₂ (φ n)) → 0
lemma A₁e₁_A₂e₂_subseq_converge_zero : Tendsto (fun n => A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))) atTop (𝓝 0) := by
   rw [← add_zero 0]
   apply Tendsto.add (A₁e₁_subseq_converge_zero fullrank₁ fullrank₂) (A₂e₂_subseq_converge_zero fullrank₁ fullrank₂)

-- ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖ → 0
lemma nrm_A₁e₁_A₂e₂_subseq_converge_zero: Tendsto (fun n => ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖) atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.1; apply A₁e₁_A₂e₂_subseq_converge_zero

-- ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖^2 → 0
lemma sqnrm_A₁e₁_A₂e₂_subseq_converge_zero: Tendsto (fun n => ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖^2) atTop (𝓝 0) := by
...
lemma const_sqnrm_A₁e₁_A₂e₂_subseq_converge_zero: Tendsto (fun n => ((max (1-τ) (1-(1/τ)))*ρ) * ‖A₁ (e₁ (φ n)) + A₂ (e₂ (φ n))‖^2) atTop (𝓝 0) := by
   rw [← mul_zero]
   apply Filter.Tendsto.const_mul ((max (1-τ) (1-(1/τ)))*ρ) (sqnrm_A₁e₁_A₂e₂_subseq_converge_zero fullrank₁ fullrank₂)

-- byf
-- Φ (φ n) → 0
lemma Φ_subseq_converge_zero : Tendsto (fun n => Φ (φ n)) atTop (𝓝 0) := by
...
lemma Φ_ge :∀ n , 0 ≤ (fun _ : ℕ ↦ (⨅ i, (fun n ↦ Φ (n + 1)) i)) n := by
   intro n
   simp only
...
lemma Φ_bddbelow : BddBelow (range (fun n ↦ Φ (n + 1))) := by
...
lemma Φ_le :∀ n : ℕ , (⨅ i, (fun n ↦ Φ (n + 1)) i) ≤ Φ (φ n.succ) := by
   intro n
...
lemma A₂e₂_pow_converge_zero : Tendsto (fun n => ‖A₂ (e₂ n)‖^2) atTop (𝓝 0) := by
...
lemma A₂e₂_converge_zero : Tendsto (fun n => ‖A₂ (e₂ n)‖) atTop (𝓝 0) := by
...
--dyx xzx
lemma CA₁e₁_converge_zero (C : ℝ) : Tendsto (fun n => C * ‖A₁ (e₁ n)‖) atTop (𝓝 0) := by
...
lemma e₁_converge_zero : Tendsto e₁ atTop (𝓝 0) := by
...
-- hpf
lemma ey_le_Φ (n : ℕ) : (1 / (τ * ρ)) * ‖ey n‖ ^ 2 ≤ Φ n := by
   dsimp [Φ, Ψ]
...
-- hpf
lemma ey_le_Φ' (n : ℕ) : ‖ey n‖ ^ 2 ≤ (τ * ρ) * Φ n := by
...
-- byf
lemma const_φ_converge_zero : Tendsto (fun n => (τ * ρ) * Φ n) atTop (𝓝 0) := by
   rw [← mul_zero (τ * ρ)]
   apply Filter.Tendsto.const_mul (τ * ρ) (Φ_converge_zero fullrank₁ fullrank₂)

lemma ey_sqnrm_converge_zero : Tendsto (fun n => ‖ey n‖^2)  atTop (𝓝 0) := by
   apply squeeze_zero_norm
   have (n : ℕ) : ‖‖ey n‖ ^ 2‖ ≤ (τ * ρ) * Φ n := by simp [ey_le_Φ']
   apply this; apply const_φ_converge_zero

lemma ey_nrm_converge_zero : Tendsto (fun n => ‖ey n‖)  atTop (𝓝 0) := by
   rw [← Real.sqrt_zero]
...
lemma y_converge : Tendsto y atTop (𝓝 y'') := by
